...
void ODBCSQLServerTest::testSimpleAccess()
{
    ...
    try
    {
        *_pSession << "SELECT count(*) FROM sys.tables WHERE name = 'Person'", into(count), use(tableName), now;
    }
...
void ODBCSQLServerTest::recreatePersonBLOBTable()
{
    ...
    try
    {
        *_pSession << "CREATE TABLE Person (LastName VARCHAR(30), FirstName VARCHAR(30), Address VARCHAR(30), Image VARBINARY(MAX))", now;
    }
    catch(ConnectionException& ce)
    {
        std::cout << ce.toString() << std::endl;
...
void ODBCSQLServerTest::recreateBoolsTable()
{
    ...
    try
    {
        *_pSession << "CREATE TABLE Strings (str BIT)", now;
    }
    catch(ConnectionException& ce)
    {
        std::cout << ce.toString() << std::endl;
...
void ODBCSQLServerTest::recreateVectorTable()
{
    ...
    try
    {
        *_pSession << "CREATE TABLE Vector (i0 INTEGER)", now;
    }
    catch(ConnectionException& ce)
    {
        std::cout << ce.toString() << std::endl;
        fail ("recreateVectorTable()");
    }
    catch(StatementException& se)
    {
        std::cout << se.toString() << std::endl;
...
void ODBCSQLServerTest::recreateVectorsTable()
{
    ...
    try
    {
        *_pSession << "CREATE TABLE Vectors (int0 INTEGER, flt0 FLOAT, str0 VARCHAR(30))", now;
    }
    catch(ConnectionException& ce)
    {
        std::cout << ce.toString() << std::endl;
